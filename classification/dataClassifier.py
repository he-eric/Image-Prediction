...
def enhancedFeatureExtractorFace(datum):
  ...
  for x in range(FACE_DATUM_WIDTH/5):
    for y in range(FACE_DATUM_HEIGHT/5):
      features[(x,y)] = featureValueExtractorFace(datum, x, y, 5)

  return features

def featureValueExtractorFace(datum, x, y, block):
  for a in range(block):
    for b in range(block):
      if datum.getPixel(x*block+a, y*block+b) > 0:
        return 1
  return 0
